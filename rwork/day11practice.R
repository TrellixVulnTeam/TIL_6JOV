...
Q3<-mpg %>% 
  group_by(manufacturer) %>% 
  summarise(mean.hwy_manu=mean(cty)) %>% 
...
mutate(grade=ifelse(propNoadult>=40,'large',
                    ifelse(propNoadult<30,'small','middle'))) %>% 
  group_by(grade) %>% 
  summarise(grade=n())
...
midwest7 %>% 
  mutate(asia_per=popasian/poptotal*100) %>% 
  select(state,county,asia_per) %>% 
  arrange(asia_per) %>% 
  head(10)
